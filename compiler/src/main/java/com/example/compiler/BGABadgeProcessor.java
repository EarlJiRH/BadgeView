...
public class BGABadgeProcessor extends AbstractProcessor {
    ...
    private static final String PACKAGE_NAME = "com.example.badgeview";
     ...
     *
     * @param annotations              所有的由该 Processor 处理，并待处理的 Annotations「属于该 Processor 处理的注解，但并未被使用，不存在与这个集合里」
     * @param roundEnv                 表示当前或是之前的运行环境，可以通过该对象查找到注解
     * @return 表示这组 Annotation 是否被这个 Processor 消费，如果消费「返回 true」后续子的 Processor 不会再对这组 Annotation 进行处理
     */
    @Override
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        Set<? extends Element> elements = roundEnv.getElementsAnnotatedWith(BGABadge.class);
        ...
        try {
            ...
            messager.printMessage(Diagnostic.Kind.ERROR, "Exception occurred when generating class file.");
            e.printStackTrace();
        }
        messager.printMessage(Diagnostic.Kind.NOTE,
    ...
    private void generate(Set<String> viewClassSet) throws IllegalAccessException, IOException {
        messager.printMessage(Diagnostic.Kind.NOTE, "生成 " + viewClassSet.size() + " 个");
        for (String clazz : viewClassSet) {
            ...
            messager.printMessage(Diagnostic.Kind.NOTE, clazz + " ====> " + className);
